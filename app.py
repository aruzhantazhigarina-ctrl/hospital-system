from flask import Flask, request, jsonify, render_template
import sqlite3
import datetime

app = Flask(__name__)
DB = "hospital.db"


# ---------------- DB ----------------
def get_db():
    conn = sqlite3.connect(DB)
    conn.row_factory = sqlite3.Row
    return conn


def init_db():
    conn = get_db()
    cur = conn.cursor()

    # tables
    cur.execute("""
    CREATE TABLE IF NOT EXISTS districts (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT,
        address TEXT
    )""")

    cur.execute("""
    CREATE TABLE IF NOT EXISTS doctors (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        full_name TEXT,
        specialization TEXT,
        cabinet TEXT,
        district_id INTEGER
    )""")

    cur.execute("""
    CREATE TABLE IF NOT EXISTS patients (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        full_name TEXT,
        iin TEXT,
        phone TEXT,
        gender TEXT
    )""")

    cur.execute("""
    CREATE TABLE IF NOT EXISTS appointments (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        doctor_id INTEGER,
        patient_id INTEGER,
        date TEXT,
        time TEXT,
        UNIQUE(doctor_id, date, time)
    )""")

    cur.execute("""
    CREATE TABLE IF NOT EXISTS medical_records (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        doctor_id INTEGER,
        patient_id INTEGER,
        diagnosis TEXT
    )""")

    # reset data
    cur.execute("DELETE FROM districts")
    cur.execute("DELETE FROM doctors")

    # districts
    cur.executemany("""
    INSERT INTO districts (name, address) VALUES (?, ?)
    """, [
        ("Есиль", "Мангилик Ел 52"),
        ("Сарыарка", "Тауелсиздик 1-25"),
        ("Алматы", "Абая 10")
    ])

    # doctors
    cur.executemany("""
    INSERT INTO doctors (full_name, specialization, cabinet, district_id)
    VALUES (?, ?, ?, ?)
    """, [
        ("Сериков Нурлан", "Терапевт", "101", 1),
        ("Касымов Айбек", "Хирург", "102", 1),
        ("Жумабеков Ержан", "Ортодонт", "103", 1),
        ("Алиева Жанар", "Гнатолог", "104", 1),
        ("Омаров Руслан", "Имплантолог", "105", 1),
        ("Тлеубергенов Данияр", "ЧЛХ", "106", 1),

        ("Бекетов Арман", "Терапевт", "201", 2),
        ("Нурпеисов Канат", "Хирург", "202", 2),
        ("Садыков Марат", "Ортодонт", "203", 2),
        ("Кенжебаева Алия", "Гнатолог", "204", 2),
        ("Турсунов Ермек", "Имплантолог", "205", 2),
        ("Жаксылыков Дастан", "ЧЛХ", "206", 2),

        ("Айдаров Тимур", "Терапевт", "301", 3),
        ("Смагулова Динара", "Хирург", "302", 3),
        ("Исабеков Нурсултан", "Ортодонт", "303", 3),
        ("Калиева Айгуль", "Гнатолог", "304", 3),
        ("Мухамеджанов Олжас", "Имплантолог", "305", 3),
        ("Рахимов Ерасыл", "ЧЛХ", "306", 3),
    ])

    conn.commit()
    conn.close()


init_db()


# ---------------- PAGES ----------------
@app.route('/')
def home():
    return render_template("index.html")


@app.route('/admin')
def admin():
    return render_template("admin.html")


# ---------------- API ----------------
@app.route('/districts')
def districts():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM districts")
    data = cur.fetchall()
    conn.close()
    return jsonify([dict(x) for x in data])


@app.route('/doctors/<int:district_id>')
def doctors(district_id):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM doctors WHERE district_id=?", (district_id,))
    data = cur.fetchall()
    conn.close()
    return jsonify([dict(x) for x in data])


@app.route('/available/<int:doctor_id>/<date>')
def available(doctor_id, date):
    conn = get_db()
    cur = conn.cursor()

    slots = ["09:00","10:00","11:00","12:00","13:00","14:00","15:00","16:00","17:00"]

    cur.execute("""
    SELECT time FROM appointments
    WHERE doctor_id=? AND date=?
    """, (doctor_id, date))

    busy = [x["time"] for x in cur.fetchall()]
    free = [t for t in slots if t not in busy]

    conn.close()
    return jsonify(free)


@app.route('/add', methods=['POST'])
def add():
    data = request.json

    conn = get_db()
    cur = conn.cursor()

    # weekend check
    day = datetime.datetime.strptime(data['date'], "%Y-%m-%d").weekday()
    if day >= 5:
        return "WEEKEND"

    # busy check
    cur.execute("""
    SELECT * FROM appointments
    WHERE doctor_id=? AND date=? AND time=?
    """, (data['doctor_id'], data['date'], data['time']))

    if cur.fetchone():
        return "TIME_BUSY"

    # patient
    cur.execute("SELECT id FROM patients WHERE iin=?", (data['iin'],))
    p = cur.fetchone()

    if p:
        patient_id = p["id"]
    else:
        cur.execute("""
        INSERT INTO patients (full_name, iin, phone, gender)
        VALUES (?, ?, ?, ?)
        """, (data['name'], data['iin'], data['phone'], data['gender']))
        patient_id = cur.lastrowid

    # appointment
    cur.execute("""
    INSERT INTO appointments (doctor_id, patient_id, date, time)
    VALUES (?, ?, ?, ?)
    """, (data['doctor_id'], patient_id, data['date'], data['time']))

    conn.commit()
    conn.close()

    return "OK"


@app.route('/all_appointments')
def all_appointments():
    conn = get_db()
    cur = conn.cursor()

    cur.execute("""
    SELECT a.id,
           d.full_name AS doctor,
           d.specialization,
           d.cabinet,
           p.full_name AS patient,
           p.iin,
           a.date,
           a.time
    FROM appointments a
    JOIN doctors d ON a.doctor_id=d.id
    JOIN patients p ON a.patient_id=p.id
    """)

    data = cur.fetchall()
    conn.close()
    return jsonify([dict(x) for x in data])


@app.route('/records_by_iin/<iin>')
def records(iin):
    conn = get_db()
    cur = conn.cursor()

    cur.execute("""
    SELECT mr.diagnosis,
           d.full_name AS doctor
    FROM medical_records mr
    JOIN doctors d ON mr.doctor_id=d.id
    JOIN patients p ON mr.patient_id=p.id
    WHERE p.iin=?
    """, (iin,))

    data = cur.fetchall()
    conn.close()
    return jsonify([dict(x) for x in data])


@app.route('/add_record', methods=['POST'])
def add_record():
    data = request.json

    conn = get_db()
    cur = conn.cursor()

    cur.execute("""
    INSERT INTO medical_records (doctor_id, patient_id, diagnosis)
    VALUES (?, ?, ?)
    """, (data['doctor_id'], data['patient_id'], data['diagnosis']))

    conn.commit()
    conn.close()

    return "OK"


if __name__ == "__main__":
    app.run(debug=True)
